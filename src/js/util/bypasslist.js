export default function(app) {
  const storageKeys = {userrk: "bypasslist:customlist", poprk:"bypasslist:popularrules"},
        {storage, regionlist} = app.util,
        trimUserRules = (rules) => rules.map(e => e.trim()).filter(e => e.length > 0),
        enabledRules = new Map([
  ...
  this.enabledPopularRules = () => {
    const enabledRulesByName = Array.from(enabledRules.keys()),
          popularRulesByName = this.popularRulesByName()
  ...
  if(storage.hasItem(poprk) && storage.getItem(poprk).length > 0)
    stringToArray(storage.getItem(poprk)).forEach((name) => this.enablePopularRule(name))
